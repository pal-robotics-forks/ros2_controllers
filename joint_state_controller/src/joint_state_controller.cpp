...
JointStateController::update()
{
  ...
  for (auto joint_index = 0ul; joint_index < joint_names_.size(); ++joint_index) {
    ...
    /// @todo somehow don't use hardcoded indexes directly
    update_value(joint_state_msg_.position[joint_index], joint_handles_[joint_index][0]);
    update_value(joint_state_msg_.velocity[joint_index], joint_handles_[joint_index][1]);
    update_value(joint_state_msg_.effort[joint_index], joint_handles_[joint_index][2]);
